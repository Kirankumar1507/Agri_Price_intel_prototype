...
def fetch_prices(state: str, commodity_api: str, days: int = WINDOW_DAYS) -> list[dict]:
    ...
    with instrumentation.trace_call(
        "mandi.fetch_prices", {"state": state, "commodity": commodity_api}
    ) as _tc:
...
def fetch_all_mandis_for_state(state: str) -> list[dict]:
    ...
    """
    from src import instrumentation
    with instrumentation.trace_call("mandi.fetch_all", {"state": state}) as _tc:
